...
Mixed model using a Delayed Constant Treatment (DCT) model. This model assumes the treatment effect during the first `w` exposure time periods is a nuisance "washout" effect, after which the treatment stabilizes to a constant effect $\delta$. Set `w` via `params()`.
...
The returned `te_est` is the estimate of $\delta$ — the constant treatment effect after the washout period. The washout-period coefficients ($\delta_1, \delta_2$) are estimated internally but treated as nuisance parameters.
